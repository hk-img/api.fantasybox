    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h1 class="page-header-title">Company Manager</h1>
                </div>
            </div>
            <!-- End Row -->
        </div>
        ...
        <div class="card">
            ...
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <h4 class="card-header-title">View Company</h4>
                </div>
                <div class="mb-2 mb-md-0">
                    <form>
                        <!-- Search -->
                ...
                <div class="tom-select-custom">
                    <select class="js-select form-select" id="fantasy_type" name="fantasy_type"
                        onchange="getcustomexchange(this.value)"
                        ...
                        @foreach ($exchanges as $val)
                            <option <?php if ($exchangecode == $val->code) {
                                echo 'selected';
                            } ?> value="{{ $val->code }}"
                ...
                <table id="datatable"
                    ...
                    <thead class="thead-light">
                        <tr class="text-center">
                            ...
                            <th class="table-column-ps-0">Symbol</th>
                            <th class="table-column-ps-0">Type</th>
                            <th class="table-column-ps-0">Image</th>
                            <th class="table-column-ps-0">Stock Exchange</th>
                            <th class="table-column-ps-0">Currency</th>
                        </tr>
                    </thead>
                        ...
                        @foreach ($collection as $item)
                            <tr class="text-center">
                                ...
                                <td>
                                    ...
                                    @else
                                        <a href="{{ route('series.actdect', base64_encode($item->id)) }}" type="button"
                                            class="btn btn-success btn-sm">
                                            Enable
                                        </a>
                                    @endif
                                    <a href="{{route('getcompanyprofile',$item->symbol)}}" type="button"
                                        class="btn btn-success btn-sm text-white">
                                        <i class="bi bi-arrow-clockwise me-1"></i> Update Profile
                                    </a>
                                    <a href="{{route('getcompanyprice',$item->symbol)}}" type="button"
                                        class="btn btn-success btn-sm text-white">
                                        <i class="bi bi-arrow-clockwise me-1"></i>Price Update
                                    </a>
                                </td>
                                ...
                                <td class="table-column-ps-0">
                                    <div class="ms-3">
                                        <span class="d-block h5 text-inherit mb-0">{{ $item->symbol }}</span>
                                    </div>
                                </td>
                                <td class="table-column-ps-0">
                                    <div class="ms-3">
                                ...
                                <td class="table-column-ps-0">
                                    <div class="ms-3">
                                        @if (strpos($item->images, 'https') === 0)
                                            <img class="avatar avatar-lg" src="{{ $item->images }}" class="img-fluid"
                                                style="max-height: 50px; max-width: 50px;"
                                                onerror="this.src='{{ asset('public/settings/' . SettingsTrait::settings('team_image')) }}'">
                                        @else
                                            <img class="avatar avatar-lg" src="{{ asset('public/' . $item->images) }}"
                                ...
                                <td class="table-column-ps-0">
                                    <div class="ms-3">
                                        <span class="d-block h5 text-inherit mb-0">{{ $item->exchange }}</span>
                                    </div>
                                </td>
                                <td class="table-column-ps-0">
                                    <div class="ms-3">
                                        <span class="d-block h5 text-inherit mb-0">{{ $item->currency }}</span>
                                    </div>
                                </td>
                            </tr>
                            @php
...
@endsection
<script>
    function getcustomexchange(value) {
        // Encode the selectedName using Base64
        var url = "{{ route('stock.company') }}";
        url = url + "?exchange=" + value; // Append the matchkey parameter
