    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h1 class="page-header-title">Stock Match Manager</h1>
                </div>
            </div>
            <!-- End Row -->
        </div>
                                ...
                                <select class="js-select form-select" id="matchkey" name="matchkey"
                                    ...
                                    @foreach ($launchedmatch as $val)
                                        <option <?php if ($matchkey == $val->matchkey) {
                                            ...
                                            <span class="text-dark">{{ date('l,',strtotime($val->date))}}</span><span class="text-warning">{{ date(' d-M-y',strtotime($val->date))}}</span><span class="text-success ml-2">{{ date(' h:i:s a',strtotime($val->date))}}</span></span></option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-3">
                                {{-- <a class="btn btn-warning <?php if (empty($matchkey)) {
                                    ...
                                    href="{{ route('match.importstockcontest', base64_encode($matchkey)) }}">
                                    <i class="bi bi-cloud-download-fill"></i> Import Contest
                                </a> --}}
                                <a class="btn btn-primary <?php if (empty($matchkey)) {
                                    echo 'disabled';
                                } ?>"
                                    href="{{ route('stockcustomcontest.create', ['matchkey' => $matchkey]) }}">
                                                    ...
                                                    <div class="btn-group">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end mt-1"
                                                            ...
                                                            @if ($challenge->joinedusers == 0)
                                                                ...
                                                                <a class="dropdown-item"
                                                                    href="{{ route('stockcustomcontest.edit', base64_encode($challenge->id)) }}">
                                                                    <i class="bi-pencil-fill dropdown-item-icon"></i> Edit
                                                                    Contest
                                                                </a>
                                                                <a class="dropdown-item" href="{{ route('stockcustomcontest.pricecard', base64_encode($challenge->id)) }}">
...
<script>
    function getcustommatch(value) {
        ...
        var url = "{{ route('match.stockcustomcontest') }}";
        ...
        swal({
            ...
            if (confirmed) {
                $.ajax({
                    ...
                    url: "{{ route('stockcustomcontest.deletecustomcontest') }}",
                    data: {
                        id: id
                    },
        ...
        swal({
            ...
            if (confirmed) {
                $.ajax({
                    ...
                    url: "{{ route('stockcustomcontest.cancelcontest') }}",
                    data: {
                        id: id
                    },
                    success: function(data) {
